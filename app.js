...
const RESOLUTION = '1k';
const CONCURRENCY = process.env.VERCEL ? 2 : 6;
const OUTPUT_DIR = process.env.VERCEL
  ? path.join('/tmp', 'mod.peg')
  : path.join(__dirname, 'mod.peg');
...
function requireApiConfig(res) {
  if (!API_BASE_URL || !API_KEY) {
    res.status(500).json({
      message:
        '服务端未配置 API_BASE_URL / API_KEY。本地请配置 .env；Vercel 请在项目 Settings → Environment Variables 中添加。'
    });
    return false;
  }
  return true;
...
async function generateOneImage(prompt, size, batchId, index, seed) {
  ...
  try {
    ...
    console.warn('写入磁盘失败（Vercel 仅 /tmp 可写）:', err.message);
  }
...
app.post('/api/generate', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      images: results,
      savedDir: process.env.VERCEL ? null : OUTPUT_DIR,
      batchId
    });
  } catch (err) {
